...
Problem statement-Monica wants to buy a keyboard and a USB drive from her favorite electronics store. The store has several models of each. Monica wants to spend as much as possible for the  items, given her budget.
...
for(i=0;i<keyboards.size();i++){
    for(j=0;j<drives.size();j++){
        if(keyboards[i]+drives[j]<=b){
            abc=max(abc,keyboards[i]+drives[j]);
        }
    }
}
return abc;
